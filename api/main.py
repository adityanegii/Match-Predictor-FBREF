...
def process(df, file=False):
    return DP.combine(DP.get_overall_averages(DP.clean_data(df), file))

def train(data, predictors, rf):
    RF.evaluate_model(RF.train(data.dropna(), predictors, rf))

def predict(data, predictors, rf):
    RF.predict(data, predictors, rf).to_csv("data/predictions.csv", index=False)
...
def main():
    ...
    next_games = data[data['date'].dt.date >= pd.to_datetime("2023-05-27").dt.date]
